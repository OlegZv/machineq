...
class TestOutputProfiles:
    ...
    def test_output_profiles_update_and_patch(self, output_profiles_api: SyncOutputProfiles):
        ...
        try:
            ...
            updated = output_profiles_api.update(profile_id, update_data)
            assert updated

            # verify the update
            fetched = output_profiles_api.get(profile_id)
            ...
            patched = output_profiles_api.patch(profile_id, patch_data)
    ...
    def test_output_profiles_update_devices_put(
        ...
        try:
            ...
            test_device = device_api.create(new_device)
            new_device.deveui = deveui2
            test_device_2 = device_api.create(new_device)

            # first try patch by adding two devices one by one
            patch_data = OutputProfileDevicesUpdate(
                devices=[test_device],
            )
            patched = output_profiles_api.add_devices(profile_id, patch_data)
            ...
            fetched_device = device_api.get(test_device)
            ...
            fetched_device_2 = device_api.get(test_device_2)
            ...
            resp = output_profiles_api.update_devices(profile_id, update)
            ...
            output_profiles_api.delete(profile_id)
            if test_device:
                device_api.delete(test_device)
            if test_device_2:
                device_api.delete(test_device_2)
